<x-app-layout>
    <form method="POST" action="{{ route('login') }}" class="w-[400px] mx-auto p-6 my-16">
        ...
        <p class="text-center text-gray-500 mb-6">
            ...
            <a
                href="{{ route('register') }}"
                class="text-sm text-lime-700 hover:text-lime-600"
            >
                create new account
            </a>
        </p>
        ...
        <div class="flex justify-between items-center mb-5">
            <div class="flex items-center">
                <input
                    id="loginRememberMe"
                    type="checkbox"
                    class="mr-3 rounded border-gray-300 text-lime-500 focus:ring-lime-500"
                />
                <label for="loginRememberMe">Remember Me</label>
            </div>
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm text-lime-700 hover:text-lime-600">
                    Forgot Password?
                </a>
            @endif
        </div>
